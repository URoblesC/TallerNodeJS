...
employee.post("/agregar", async (req, res, next) => {
    const { empleado_nombre, empleado_apellidos, empleado_telefono, empleado_correo, empleado_direccion } = req.body;
    ...
    if (empleado_nombre && empleado_apellidos && empleado_telefono && empleado_correo && empleado_direccion) {
        ...
        if (rows.affectedRows == 1) {
            return res.status(200).json({ code: 200, message: "Empleado agregado Correctamente" });
        }
